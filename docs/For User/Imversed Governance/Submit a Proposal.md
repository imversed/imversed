...
#### Real Example
[Proposal 1](https://commonwealth.im/evmos/proposal/1-airdrop-claim-mission) was representative of one of four core network activities that users had to participate in to claim tokens from the Imversed Rektdrop.

```json
{
  "title": "Airdrop Claim Mission",
  "description": "Vote to claim",
  "deposit": "10000000000000000000aivm"
}
```
...
4. **Amount** - the amount of funding that the recipient will receive in atto-IVM (`aivm`)
5. **Deposit** - the amount that will be contributed to the deposit (in `aivm`) from the account submitting the proposal

#### Made-Up Example
In this simple example (below), a network explorer will list the governance proposal as a `CommunityPoolSpendProposal`. When an observer selects the proposal, they'll see the description. Not all explorers will show the recipient and amount, so ensure that you verify that the description aligns with the what the governance proposal is programmed to enact. If the description says that a certain address will receive a certain number of IVM, it should also be programmed to do that, but it's possible that that's not the case (accidentally or otherwise).

The `amount` is `1000000000000000000aivm`. This is equal to 1 IVM, so `recipient` address `evmos1mx9nqk5agvlsvt2yc8259nwztmxq7zjq50mxkp` will receive 1 IVM if this proposal is passed.

The `deposit` of `192000000000000000000aivm` results in 192 IVM being used from the proposal submitter's account. A minimum deposit is required for a proposal to enter the voting period, and anyone may contribute to this deposit within 3 days. If the minimum deposit isn't reached before this time, the deposit amounts will be burned. Deposit amounts will also be burned if the quorum isn't met in the vote or the proposal is vetoed.
...
```json
{
  ...
  "deposit": "64000000000000000000aivm"
}
```
...
```linux
`imversed --node https://tendermint.bd.evmos.org:26657 query gov proposal 23`.
```

```json
{
  "title": "Grant proposal for Flux Protocol an oracle solution live on Imversed",
...
6. **Denom** - `aivm` (atto-IVM) will be the type of asset used as the deposit
7. **Amount** - the amount that will be contributed to the deposit (in aivm) from the account submitting the proposal
...
```linux
`imversed --node https://tendermint.bd.evmos.org:26657 query gov proposal 7`.
```
```json
{
  ...
  "changes": [
    {
      "subspace": "gov",
      "key": "depositparams",
      "value": {"mindeposit":[{"denom":"aivm","amount":"64000000000000000000"}],
      "max_deposit_period":"1209600000000000"}
    }
  ],
  "deposit": "20100000000000000000aivm"
}
```

The deposit `denom` is `aivm` and `amount` is `20100000000000000000`. Therefore, a deposit of 20.1 IVM will be included with this proposal. At the time, the IVM mainnet had a 10 IVM minimum deposit, so this proposal was put directly into the voting period (and subsequently passed). The minimum deposit amount is currently 192 IVM. There is a minimum deposit required for a proposal to enter the voting period, and anyone may contribute to this deposit within a 3-day period. If the minimum deposit isn't reached before this time, the deposit amounts will be burned.
...
6. `--chain-id evmos_9001-2` is Imversed Mainnet. For current and past chain-id's, please look at the Chain ID documentation.
...
After posting your transaction, your command line interface (`imversed`) will provide you with the transaction's hash, which you can either query using `imversed` or by searching the transaction hash using [Mintscan](https://www.mintscan.io/evmos) or any block explorer.
...
```linux
imversed q gov proposals --depositor evmos1hxv7mpztvln45eghez6evw2ypcw4vjmsmr8cdx
```
...
In our case above, the `<proposal-id>` would be 59 as queried earlier. The `<deposit>` is written as `500000aivm`, just like the example above.
...
* the deposit denomination is in `'ativm'` instead of `'aivm'`
