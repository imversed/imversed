...
func TestCreateCurrency(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdCreateCurrency(), args)
			if tc.err != nil {
...
func TestUpdateCurrency(t *testing.T) {
	...
	_, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdCreateCurrency(), args)
	require.NoError(t, err)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			args = append(args, fields...)
			args = append(args, tc.args...)
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdUpdateCurrency(), args)
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
...
func TestDeleteCurrency(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			args = append(args, tc.args...)
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdDeleteCurrency(), args)
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
