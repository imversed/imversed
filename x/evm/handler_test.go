...
func (suite *EvmTestSuite) DoSetupTest(t require.TestingT) {
	...
	suite.app = app.Setup(checkTx, func(app *app.ImversedApp, genesis simapp.GenesisState) simapp.GenesisState {
...
func (suite *EvmTestSuite) TestERC20TransferReverted() {
	...
	for _, tc := range testCases {
		suite.Run(tc.msg, func() {
			...
			suite.Require().NoError(err)
			_, err = k.DeductTxCostsFromUserBalance(suite.ctx, *tx, txData, "nimv", true, true, true)
			suite.Require().NoError(err)
