...
func (suite *KeeperTestSuite) measureJoinPoolGas(
	...
	err := suite.app.PoolsKeeper.JoinPool(suite.ctx, addr, poolID, shareOutAmountMax, maxCoins)
...
func (suite *KeeperTestSuite) TestRepeatedJoinPoolDistinctDenom() {
	...
	for i := 2; i < denomNumber; i++ {
		err := suite.app.PoolsKeeper.JoinPool(suite.ctx, defaultAddr, uint64(i), minShareOutAmount, sdk.Coins{})
		suite.Require().NoError(err)
	}
