...
func (suite *KeeperTestSuite) TestQueryTotalShares() {
	...
	pool, err := suite.app.PoolsKeeper.GetPool(suite.ctx, poolId)
	suite.Require().NoError(err)
	err = suite.app.PoolsKeeper.MintPoolShareToAccount(suite.ctx, pool, acc1, types.OneShare.MulRaw(10))
	suite.Require().NoError(err)
	suite.Require().NoError(suite.app.PoolsKeeper.SetPool(suite.ctx, pool))
