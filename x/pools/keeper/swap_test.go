...
func (suite *KeeperTestSuite) TestActivePoolSwap() {
	...
	for _, tc := range testCases {
		...
		for _, acc := range []sdk.AccAddress{acc1, acc2, acc3} {
			...
			if tc.expectPass {
				_, _, err = suite.app.PoolsKeeper.SwapExactAmountIn(suite.ctx, acc1, poolId, foocoin, "bar", sdk.ZeroInt())
				suite.Require().NoError(err)
				_, _, err = suite.app.PoolsKeeper.SwapExactAmountOut(suite.ctx, acc1, poolId, "bar", sdk.NewInt(1000000000000000000), foocoin)
				suite.Require().NoError(err)
			} else {
				_, _, err = suite.app.PoolsKeeper.SwapExactAmountIn(suite.ctx, acc1, poolId, foocoin, "bar", sdk.ZeroInt())
